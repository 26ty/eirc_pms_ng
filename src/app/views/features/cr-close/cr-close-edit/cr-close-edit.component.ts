...
@Component({
  selector: 'app-cr-close-edit',
  templateUrl: './cr-close-edit.component.html',
  styleUrls: ['./cr-close-edit.component.scss']
})
export class CrCloseEditComponent implements OnInit {
  ...
  getCrReturnList_J(account: any, userId: any): void {
    ...
    this.HttpApiService.getDepartmentCloseCrReturnList(account, userId).subscribe(res => {
      ...
      for (let i in this.userCrReturnData_J.body) {
        if (this.cd_id == this.userCrReturnData_J.body[i].cd_id && this.bonita_task_id == this.userCrReturnData_J.body[i].bonita_task_id) {
          this.CRDatas.push(this.userCrReturnData_J.body[i])
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.HttpApiService.ReviewCRForm(this.userJson.account, this.bonita_task_id, submitA1).subscribe(res => {
          console.log("res", res)
          //更新客需單資料庫
          this.updateCustomerRequest('結案中(副總)')
          /*新增審核紀錄 */
          this.uploadTransactionRecordRequests(this.cd_id, '單位主管結案簽核')
