...
@Component({
  selector: 'app-cr-sign-edit',
  templateUrl: './cr-sign-edit.component.html',
  styleUrls: ['./cr-sign-edit.component.scss']
})
export class CrSignEditComponent implements OnInit {
  ...
  getCrReturnList_G(account: any, userId: any): void {
    ...
    this.HttpApiService.getBusinessCrReturnList(account, userId).subscribe(res => {
      ...
      //要改
      for (let i in this.userCrReturnData_G.body) {
        if (this.cd_id == this.userCrReturnData_G.body[i].cd_id && this.bonita_task_id == this.userCrReturnData_G.body[i].bonita_task_id) {
          this.CRDatas.push(this.userCrReturnData_G.body[i])
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.HttpApiService.ReviewCRForm(this.userJson.account, this.bonita_task_id, submitA1).subscribe(res => {
          ...
          /*新增審核紀錄 */
          this.uploadTransactionRecordRequests(this.cd_id, '業務簽核')
