...
@Component({
  selector: 'app-cr-close-inform-edit',
  templateUrl: './cr-close-inform-edit.component.html',
  styleUrls: ['./cr-close-inform-edit.component.scss']
})
export class CrCloseInformEditComponent implements OnInit {
  ...
  getCrReturnList_M(account: any, userId: any): void {
    ...
    this.HttpApiService.getCountersignCloseCrReturnList(account, userId).subscribe(res => {
      ...
      for (let i in this.userCrReturnData_M.body) {
        if (this.cd_id == this.userCrReturnData_M.body[i].cd_id && this.bonita_task_id == this.userCrReturnData_M.body[i].bonita_task_id) {
          this.CRDatas.push(this.userCrReturnData_M.body[i])
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire(
            ...
            if (result.isConfirmed) {
              location.href = '/main/cr-close-inform'
            }
          })
  ...
  updateCustomerRequest(): void {
    ...
    //填料???
    if (this.CRDatas[0].fill == 'true') {
      customerRequestData['fill'] = true;
    } else if (this.CRDatas[0].fill == 'false') {
      customerRequestData['fill'] = false;
    }
