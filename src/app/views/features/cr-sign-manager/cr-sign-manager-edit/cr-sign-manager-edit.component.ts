...
@Component({
  selector: 'app-cr-sign-manager-edit',
  templateUrl: './cr-sign-manager-edit.component.html',
  styleUrls: ['./cr-sign-manager-edit.component.scss']
})
export class CrSignManagerEditComponent implements OnInit {
  ...
  getCrReturnList_H(account: any, userId: any): void {
    ...
    this.HttpApiService.getBusinessManagerCrReturnList(account, userId).subscribe(res => {
      ...
      for (let i in this.userCrReturnData_H.body) {
        if (this.cd_id == this.userCrReturnData_H.body[i].cd_id && this.bonita_task_id == this.userCrReturnData_H.body[i].bonita_task_id) {
          this.CRDatas.push(this.userCrReturnData_H.body[i])
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.HttpApiService.ReviewCRForm(this.userJson.account, this.bonita_task_id, submitA1).subscribe(res => {
          ...
          /*新增審核紀錄 */
          this.uploadTransactionRecordRequests(this.cd_id, '單位主管簽核')
        ...
        Swal.fire(
          ...
          if (result.isConfirmed) {
            location.href = '/main/cr-sign-manager'
          }
        })
