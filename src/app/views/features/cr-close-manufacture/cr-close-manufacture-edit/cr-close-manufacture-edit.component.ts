...
@Component({
  selector: 'app-cr-close-manufacture-edit',
  templateUrl: './cr-close-manufacture-edit.component.html',
  styleUrls: ['./cr-close-manufacture-edit.component.scss']
})
export class CrCloseManufactureEditComponent implements OnInit {
  ...
  getCrReturnList_N(account: any, userId: any): void {
    ...
    this.HttpApiService.getPmCloseCrReturnList(account, userId).subscribe(res => {
      ...
      for (let i in this.userCrReturnData_N.body) {
        if (this.cd_id == this.userCrReturnData_N.body[i].cd_id && this.bonita_task_id == this.userCrReturnData_N.body[i].bonita_task_id) {
          this.CRDatas.push(this.userCrReturnData_N.body[i])
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire(
            ...
            if (result.isConfirmed) {
              location.href = '/main/cr-close-manufacture'
            }
          })
  ...
  updateCustomerRequest(): void {
    ...
    customerRequestData['status'] = '已結案';
