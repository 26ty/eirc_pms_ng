...
@Component({
  selector: 'app-cr-close-top-edit',
  templateUrl: './cr-close-top-edit.component.html',
  styleUrls: ['./cr-close-top-edit.component.scss']
})
export class CrCloseTopEditComponent implements OnInit {
  ...
  countersignColumn: string[] = ['name', 'remark', 'date_for_completion'];
  ...
  getCrReturnList_L(account: any, userId: any): void {
    ...
    this.HttpApiService.getTopCloseCloseCrReturnList(account, userId).subscribe(res => {
      ...
      console.log("L 筆數(總經理結案審核)", this.userCrReturnData_L.body.length)
      ...
      for (let i in this.userCrReturnData_L.body) {
        if (this.cd_id == this.userCrReturnData_L.body[i].cd_id && this.bonita_task_id == this.userCrReturnData_L.body[i].bonita_task_id) {
          this.CRDatas.push(this.userCrReturnData_L.body[i])
          console.log("this.CRDatas", this.CRDatas)

        }
      }

      if (this.CRDatas[0].date_for_result == "0001-01-01T00:00:00Z") {
        const today = new Date
        this.CRDatas[0].date_for_result = today
      }
    })
  }
  ...
  updateCustomerRequest(status: any): void {
    ...
    if (status == "已結案") {
      customerRequestData['status'] = status + '(' + this.CRDatas[0].result_status + ')';
    } else {
    ...
    const date = new Date();
    customerRequestData['date_for_actual_done'] = date;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.HttpApiService.ReviewCRForm(this.userJson.account, this.bonita_task_id, submitA1).subscribe(res => {
          console.log("res", res)
          //更新客需單資料庫
          this.updateCustomerRequest('已結案')
          /*新增審核紀錄 */
          this.uploadTransactionRecordRequests(this.cd_id, '總經理結案簽核')
        ...
        Swal.fire(
          ...
          if (result.isConfirmed) {
            location.href = '/main/cr-close-top'
          }
        })
