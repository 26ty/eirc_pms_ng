...
@Component({
  ...
  templateUrl: './cr-return-edit.component.html',
  styleUrls: ['./cr-return-edit.component.scss']
})
export class CrReturnEditComponent implements OnInit {
  ...
  getCrReturnList_A1(account: any, userId: any): void {
    ...
    this.HttpApiService.getCrReturnList(account, userId).subscribe(res => {
      ...
      for (let i in this.userCrReturnData_A1.body) {
        if (this.cd_id == this.userCrReturnData_A1.body[i].cd_id && this.bonita_task_id == this.userCrReturnData_A1.body[i].bonita_task_id) {
          this.CRDatas.push(this.userCrReturnData_A1.body[i])
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.HttpApiService.ReviewCRForm(this.userJson.account, this.bonita_task_id, submitA1).subscribe(res => {
          console.log("res", res)
          //更新客需單資料庫
          this.updateCustomerRequest("副總審核中")
          /*新增審核紀錄 */
          this.uploadTransactionRecordRequests(this.cd_id, '單位主管審核')
