...
@Component({
  selector: 'app-cr-close-director',
  templateUrl: './cr-close-director.component.html',
  styleUrls: ['./cr-close-director.component.scss']
})
export class CrCloseDirectorComponent implements OnInit {


  displayedColumns: string[] = ['create_time', 'code', 'demand_content', 'salesman_name', 'action_edit'];
  ...
  getCrReturnList_K(account: any, userId: any): void {
    ...
    this.HttpApiService.getDirectorCloseCloseCrReturnList(account, userId).subscribe(res => {
      ...
      this.showData(this.userCrReturnData_K.body)
    })
  }
