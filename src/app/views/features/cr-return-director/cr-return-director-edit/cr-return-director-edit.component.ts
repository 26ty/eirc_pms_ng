...
@Component({
  selector: 'app-cr-return-director-edit',
  templateUrl: './cr-return-director-edit.component.html',
  styleUrls: ['./cr-return-director-edit.component.scss']
})
export class CrReturnDirectorEditComponent implements OnInit {
  ...
  getCrReturnList_A2(account: any, userId: any): void {
    ...
    this.HttpApiService.getDirectorCrReturnList(account, userId).subscribe(res => {
      ...
      console.log("客需單待審核筆數(業務處副總審核)", this.userCrReturnData_A2.body.length)
      this.crRerurnTotal_A2 = this.userCrReturnData_A2.body.length
      console.log("客需單待審核(業務處副總審核)", this.userCrReturnData_A2.body)

      //this.showData(this.userCrReturnData_A1.body)

      for (let i in this.userCrReturnData_A2.body) {
        if (this.cd_id == this.userCrReturnData_A2.body[i].cd_id && this.bonita_task_id == this.userCrReturnData_A2.body[i].bonita_task_id) {
          this.CRDatas.push(this.userCrReturnData_A2.body[i])
          console.log("this.CRDa", this.CRDatas)
        }
      }
    })
  }
    ...
    this.HttpApiService.getCountersignRequest(this.cd_id).subscribe(
      csRequest => {
        ...
        for (let i in this.A1departmentData) {
          ...
          this.CSdepartmentList.push({ "id": this.A1departmentData[i].d_id, "name": this.A1departmentData[i].name, "ckecked": value, "cs_id": cs_id })
  ...
  submitCase(): void {
    ...
    // 副總PM選擇為非必填(220804 HTA內部人員討論結過 by ISA姊)
    // if (this.CRDatas[0].projectman_id != '00000000-0000-0000-0000-000000000000') {


    //取得專案負責人的bonita_user_id
    this.HttpApiService.getProjectBonitaUserList(this.cd_id).subscribe(userRes => {
    ...
    console.log('this.subnitA1', this.submitA1)
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.HttpApiService.ReviewCRForm(this.userJson.account, this.bonita_task_id, this.submitA1).subscribe(res => {
          ...
          //更新資料庫
          this.updateCustomerRequest("最高主管審核中")
          /*新增審核紀錄 */
          this.uploadTransactionRecordRequests(this.cd_id, '業務處副總審核')
  ...
  getDepartmentList(): void {
    this.HttpApiService.getDepartmentList()
      .subscribe(departmentRequest => {
        ...
        this.getAccountList()
        console.log('部門', departmentRequest)
