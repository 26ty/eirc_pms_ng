...
@Component({
  selector: 'app-cr-sign-director-edit',
  templateUrl: './cr-sign-director-edit.component.html',
  styleUrls: ['./cr-sign-director-edit.component.scss']
})
export class CrSignDirectorEditComponent implements OnInit {
  ...
  countersignColumn: string[] = ['name', 'remark', 'date_for_completion', 'file'];
  ...
  getCrReturnList_I(account: any, userId: any): void {
    ...
    this.HttpApiService.getBusinessDirectorCrReturnList(account, userId).subscribe(res => {
      ...
      console.log("H筆數(業務經理簽核)", this.userCrReturnData_I.body.length)
      this.crRerurnTotal_I = this.userCrReturnData_I.body.length
      console.log("H(業務經理簽核)", this.userCrReturnData_I.body)

      //this.showData(this.userCrReturnData_A1.body)

      for (let i in this.userCrReturnData_I.body) {
        if (this.cd_id == this.userCrReturnData_I.body[i].cd_id && this.bonita_task_id == this.userCrReturnData_I.body[i].bonita_task_id) {
          this.CRDatas.push(this.userCrReturnData_I.body[i])
  ...
  updateCustomerRequest(): void {
    ...
    //customerRequestData['projectman_id'] = this.projectman_id;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.HttpApiService.ReviewCRForm(this.userJson.account, this.bonita_task_id, submitA1).subscribe(res => {
          ...
          /*新增審核紀錄 */
          this.uploadTransactionRecordRequests(this.cd_id, '業務處副總簽核')
        ...
        Swal.fire(
          ...
          if (result.isConfirmed) {
            location.href = '/main/cr-sign-director'
          }
        })
