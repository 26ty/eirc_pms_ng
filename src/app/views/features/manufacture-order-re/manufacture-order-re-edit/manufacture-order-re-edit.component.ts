...
@Component({
  selector: 'app-manufacture-order-re-edit',
  templateUrl: './manufacture-order-re-edit.component.html',
  styleUrls: ['./manufacture-order-re-edit.component.scss']
})
export class ManufactureOrderReEditComponent implements OnInit {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.HttpApiService.reviewManufactureForm(this.userJson.account, this.bonita_task_id, reviewData).subscribe(res => {
          console.log("res", res)
          //更新製令資料庫
          this.updateProjectInfoManufacture('已結案')
          /*新增審核紀錄 */
          this.uploadTransactionRecordRequests(this.m_id, '儲存製令單號')
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            location.href = '/main/manufacture-order-re'
          }
        })
